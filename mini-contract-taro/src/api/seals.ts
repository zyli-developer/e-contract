...
export function getSealList(params: Record<string, any>) {
  return request({ url: '/api/v1/seal/seal-info/page', data: params })
}

/** 创建印章 */
export function createSeal(data: Record<string, any>) {
  return request({ url: '/api/v1/seal/seal-info/create', method: 'POST', data })
}

/** 删除印章 */
export function deleteSeal(id: string | number) {
  return request({ url: `/api/v1/seal/seal-info/delete?id=${id}`, method: 'DELETE' })
}
