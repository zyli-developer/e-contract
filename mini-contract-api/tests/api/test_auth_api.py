...
@pytest.mark.asyncio
async def test_sms_login_missing_fields(client):
    """短信登录缺少字段应返回 422"""
    resp = await client.post("/app-api/member/auth/sms-login", json={})
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_send_sms_code_missing_fields(client):
    """发送验证码缺少字段应返回 422"""
    resp = await client.post("/app-api/member/auth/send-sms-code", json={})
    assert resp.status_code == 422
...
async def test_update_password_requires_auth(client):
    ...
    resp = await client.put(
        ...
        json={"password": "newpass", "code": "123456"},
    )
    body = resp.json()
    assert body["code"] == 401
